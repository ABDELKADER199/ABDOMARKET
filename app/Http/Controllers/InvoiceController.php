...
class InvoiceController extends Controller
{
    ...
    public function sendInvoice(Request $request)
{
    $pdfUrl = $request->input('file_url');
    $phoneNumber = '+201552773309';  // الرقم المستلم على WhatsApp

    // تحويل URL إلى مسار محلي إذا كان محليًا
    $filePath = str_replace(url('/'), public_path(), $pdfUrl);

    // التأكد من أن الملف موجود
    if (!file_exists($filePath)) {
        return response()->json(['error' => 'الملف غير موجود', 'fileUrl' => $pdfUrl], 404);
    }

    // مسار ملف .exe
    $exePath = "D:\Projects C#\InvoicePrint\InvoicePrint\bin\Debug\net8.0\InvoicePrint.exe";

    // تنظيف المسارات وتغليفها بعلامات اقتباس مزدوجة
    $command = 'cmd /c "' . addslashes($exePath) . ' ' . addslashes($filePath) . '"';
    ...
    if ($status !== 0) {
        return response()->json([
            'error' => 'فشل في طباعة الفاتورة',
            'details' => $output
        ], 500);
    }

    return response()->json(['message' => 'تم إرسال الفاتورة للطباعة بنجاح']);
}
